...
public class KVTracerClient extends DB {
  ...
  private String cleanKey(String k) {
    String s = k.substring(14, 33); 			
	if(Character.isDigit(k.charAt(k.length() - 1)))
      s += k.substring(k.length() - 1);
	return s;
  }

  private int cleanValue(String k) {
    return k.length();
  }
      ...
      try {
        if (fields != null) {
          for (String field:fields) {
            ...
            mOpWriter.write("r " + cleanKey(encKey) + "\n");
          }
        } else {
  ...
  @Override
    public Status scan(String table, String startkey, int recordcount, Set<String> fields, Vector<HashMap<String, ByteIterator>> result) {
      try {
        if (fields != null) {
          for (String field:fields) {
            String encKey = encodeKey(table, startkey, field);
            mOpWriter.write("s " + cleanKey(encKey) + " " + recordcount + "\n");
          }
        } else {
          String prefix = encodeKey(table, startkey, "");
          HashSet<String> keys = mKeyMap.get(prefix);
          if (keys != null) {
            for (String k:keys) {
			  mOpWriter.write("s " + cleanKey(k) + " " + recordcount + "\n");
            }
          }
        }
        return Status.OK;
    ...
    public Status update(String table, String key, Map<String, ByteIterator> values) {
      try {
        for (Map.Entry<String, ByteIterator> entry : values.entrySet()) {
          ...
          mOpWriter.write("u " + cleanKey(encKey) + " " + cleanValue(value) + "\n");
        }
        return Status.OK;
      } catch (IOException exception) {
        exception.printStackTrace();
    ...
    public Status insert(String table, String key, Map<String, ByteIterator> values) {
      try {
        for (Map.Entry<String, ByteIterator> entry : values.entrySet()) {
          ...
          keys.add(encKey);
          String value = entry.getValue().toString();
          mOpWriter.write("i " + cleanKey(encKey) + " " + cleanValue(value) + "\n");
        }
        return Status.OK;
      } catch (IOException exception) {
        exception.printStackTrace();
    ...
    public Status delete(String table, String key) {
      ...
      try {
        for (String k:keys) {
          ...
          mOpWriter.write("d " + cleanKey(encKey) + "\n");
        }
      } catch (IOException exception) {
        exception.printStackTrace();
        return Status.ERROR;
